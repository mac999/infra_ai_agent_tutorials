...
def parse_arguments():
	parser = argparse.ArgumentParser(description='Convert IFC files to Neo4j graph database', formatter_class=argparse.RawDescriptionHelpFormatter, epilog=__doc__)
	parser.add_argument('--input-dir', type=str, default='input', help='Input directory containing IFC files (default: ./input)')
	parser.add_argument('--clear-db', action='store_true', help='Clear all existing data in the elements database before conversion (WARNING: This will delete all nodes and relationships!)')
	parser.add_argument('--force-clear', action='store_true', help='Force database clearing without user confirmation (use with --clear-db)')
	...
	parser.add_argument('--stats', action='store_true', help='Output statistics after conversion completion')
